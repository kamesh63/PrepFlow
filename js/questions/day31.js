...
window.QUIZ_DATA["day31"] = {
  title: "Data Engineering Masterclass - Day 31",
  ...
  questions: [
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `customers`, which SQL query calculates the cumulative sum of `price` partitioned by `amount` ordered by `timestamp`?",
        "options": [
            "SELECT CUMSUM(price) OVER (PARTITION BY amount ORDER BY timestamp) FROM customers",
            "SELECT SUM(price) OVER (PARTITION BY amount ORDER BY timestamp) FROM customers",
            "SELECT SUM(price) OVER (ORDER BY amount PARTITION BY timestamp) FROM customers",
            "SELECT SUM(price) PARTITION BY amount ORDER BY timestamp FROM customers"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `price` is greater than 1000 and drop duplicates based on `revenue`?",
        "options": [
            "df.where('price' > 1000).distinct('revenue')",
            "df.filter(F.col('price') > 1000).dropDuplicates(['revenue'])",
            "df.filter('price' > 1000).drop_duplicates('revenue')",
            "df.filter(df.price > 1000).dropDuplicates('revenue')"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `events`, which SQL query calculates the cumulative sum of `amount` partitioned by `timestamp` ordered by `session_id`?",
        "options": [
            "SELECT SUM(amount) OVER (ORDER BY timestamp PARTITION BY session_id) FROM events",
            "SELECT CUMSUM(amount) OVER (PARTITION BY timestamp ORDER BY session_id) FROM events",
            "SELECT SUM(amount) OVER (PARTITION BY timestamp ORDER BY session_id) FROM events",
            "SELECT SUM(amount) PARTITION BY timestamp ORDER BY session_id FROM events"
        ],
        "correct": [
            2
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `amount` is greater than 50 and drop duplicates based on `revenue`?",
        "options": [
            "df.filter(df.amount > 50).dropDuplicates('revenue')",
            "df.filter('amount' > 50).drop_duplicates('revenue')",
            "df.filter(F.col('amount') > 50).dropDuplicates(['revenue'])",
            "df.where('amount' > 50).distinct('revenue')"
        ],
        "correct": [
            2
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `orders`, which SQL query calculates the cumulative sum of `session_id` partitioned by `revenue` ordered by `tax`?",
        "options": [
            "SELECT SUM(session_id) PARTITION BY revenue ORDER BY tax FROM orders",
            "SELECT SUM(session_id) OVER (ORDER BY revenue PARTITION BY tax) FROM orders",
            "SELECT CUMSUM(session_id) OVER (PARTITION BY revenue ORDER BY tax) FROM orders",
            "SELECT SUM(session_id) OVER (PARTITION BY revenue ORDER BY tax) FROM orders"
        ],
        "correct": [
            3
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `timestamp` is greater than 100 and drop duplicates based on `quantity`?",
        "options": [
            "df.filter('timestamp' > 100).drop_duplicates('quantity')",
            "df.filter(F.col('timestamp') > 100).dropDuplicates(['quantity'])",
            "df.filter(df.timestamp > 100).dropDuplicates('quantity')",
            "df.where('timestamp' > 100).distinct('quantity')"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `customers`, which SQL query calculates the cumulative sum of `discount` partitioned by `price` ordered by `price`?",
        "options": [
            "SELECT SUM(discount) PARTITION BY price ORDER BY price FROM customers",
            "SELECT SUM(discount) OVER (ORDER BY price PARTITION BY price) FROM customers",
            "SELECT CUMSUM(discount) OVER (PARTITION BY price ORDER BY price) FROM customers",
            "SELECT SUM(discount) OVER (PARTITION BY price ORDER BY price) FROM customers"
        ],
        "correct": [
            3
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 3,
        "question": "Given the table `customers`, which SQL query calculates the cumulative sum of `discount` partitioned by `discount` ordered by `timestamp`?",
        "options": [
            "SELECT SUM(discount) OVER (PARTITION BY discount ORDER BY timestamp) FROM customers",
            "SELECT SUM(discount) OVER (ORDER BY discount PARTITION BY timestamp) FROM customers",
            "SELECT CUMSUM(discount) OVER (PARTITION BY discount ORDER BY timestamp) FROM customers",
            "SELECT SUM(discount) PARTITION BY discount ORDER BY timestamp FROM customers"
        ],
        "correct": [
            0
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "In Snowflake, what architectural layer is responsible for processing a query like `SELECT * FROM transactions WHERE amount = '100'`?",
        "options": [
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "In Snowflake, what architectural layer is responsible for processing a query like `SELECT * FROM users WHERE status = '50'`?",
        "options": [
            "Database Storage Layer",
            "Cloud Services Layer",
            "Metadata Layer",
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `payments`, which SQL query calculates the cumulative sum of `timestamp` partitioned by `timestamp` ordered by `user_id`?",
        "options": [
            "SELECT CUMSUM(timestamp) OVER (PARTITION BY timestamp ORDER BY user_id) FROM payments",
            "SELECT SUM(timestamp) PARTITION BY timestamp ORDER BY user_id FROM payments",
            "SELECT SUM(timestamp) OVER (ORDER BY timestamp PARTITION BY user_id) FROM payments",
            "SELECT SUM(timestamp) OVER (PARTITION BY timestamp ORDER BY user_id) FROM payments"
        ],
        "correct": [
            3
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `payments`, which SQL query calculates the cumulative sum of `revenue` partitioned by `quantity` ordered by `quantity`?",
        "options": [
            "SELECT SUM(revenue) PARTITION BY quantity ORDER BY quantity FROM payments",
            "SELECT SUM(revenue) OVER (ORDER BY quantity PARTITION BY quantity) FROM payments",
            "SELECT SUM(revenue) OVER (PARTITION BY quantity ORDER BY quantity) FROM payments",
            "SELECT CUMSUM(revenue) OVER (PARTITION BY quantity ORDER BY quantity) FROM payments"
        ],
        "correct": [
            2
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `status` is greater than 50 and drop duplicates based on `tax`?",
        "options": [
            "df.filter(F.col('status') > 50).dropDuplicates(['tax'])",
            "df.filter('status' > 50).drop_duplicates('tax')",
            "df.filter(df.status > 50).dropDuplicates('tax')",
            "df.where('status' > 50).distinct('tax')"
        ],
        "correct": [
            0
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "You have a Kafka topic `payments` with 10 partitions. If you spin up 3 consumer instances in the same consumer group, how many partitions will each consumer read from (assuming ideal balancing)?",
        "options": [
            "It depends on the producer routing key",
            "Each reads all 10 partitions",
            "Partitions are randomly assigned dynamically per message",
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `discount` is greater than 100 and drop duplicates based on `user_id`?",
        "options": [
            "df.filter('discount' > 100).drop_duplicates('user_id')",
            "df.where('discount' > 100).distinct('user_id')",
            "df.filter(F.col('discount') > 100).dropDuplicates(['user_id'])",
            "df.filter(df.discount > 100).dropDuplicates('user_id')"
        ],
        "correct": [
            2
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `transactions`, which SQL query calculates the cumulative sum of `revenue` partitioned by `discount` ordered by `amount`?",
        "options": [
            "SELECT CUMSUM(revenue) OVER (PARTITION BY discount ORDER BY amount) FROM transactions",
            "SELECT SUM(revenue) OVER (PARTITION BY discount ORDER BY amount) FROM transactions",
            "SELECT SUM(revenue) OVER (ORDER BY discount PARTITION BY amount) FROM transactions",
            "SELECT SUM(revenue) PARTITION BY discount ORDER BY amount FROM transactions"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `logs`, which SQL query calculates the cumulative sum of `status` partitioned by `timestamp` ordered by `discount`?",
        "options": [
            "SELECT SUM(status) OVER (ORDER BY timestamp PARTITION BY discount) FROM logs",
            "SELECT CUMSUM(status) OVER (PARTITION BY timestamp ORDER BY discount) FROM logs",
            "SELECT SUM(status) PARTITION BY timestamp ORDER BY discount FROM logs",
            "SELECT SUM(status) OVER (PARTITION BY timestamp ORDER BY discount) FROM logs"
        ],
        "correct": [
            3
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `events`, which SQL query calculates the cumulative sum of `user_id` partitioned by `user_id` ordered by `status`?",
        "options": [
            "SELECT SUM(user_id) OVER (ORDER BY user_id PARTITION BY status) FROM events",
            "SELECT SUM(user_id) PARTITION BY user_id ORDER BY status FROM events",
            "SELECT SUM(user_id) OVER (PARTITION BY user_id ORDER BY status) FROM events",
            "SELECT CUMSUM(user_id) OVER (PARTITION BY user_id ORDER BY status) FROM events"
        ],
        "correct": [
            2
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `users`, which SQL query calculates the cumulative sum of `price` partitioned by `revenue` ordered by `price`?",
        "options": [
            "SELECT SUM(price) OVER (ORDER BY revenue PARTITION BY price) FROM users",
            "SELECT SUM(price) PARTITION BY revenue ORDER BY price FROM users",
            "SELECT CUMSUM(price) OVER (PARTITION BY revenue ORDER BY price) FROM users",
            "SELECT SUM(price) OVER (PARTITION BY revenue ORDER BY price) FROM users"
        ],
        "correct": [
            3
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "In PySpark, how do you filter a DataFrame `df` where the column `amount` is greater than 1000 and drop duplicates based on `timestamp`?",
        "options": [
            "df.filter(df.amount > 1000).dropDuplicates('timestamp')",
            "df.filter(F.col('amount') > 1000).dropDuplicates(['timestamp'])",
            "df.where('amount' > 1000).distinct('timestamp')",
            "df.filter('amount' > 1000).drop_duplicates('timestamp')"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 1,
        "question": "Given the table `payments`, which SQL query calculates the cumulative sum of `user_id` partitioned by `timestamp` ordered by `tax`?",
        "options": [
            "SELECT SUM(user_id) PARTITION BY timestamp ORDER BY tax FROM payments",
            "SELECT SUM(user_id) OVER (PARTITION BY timestamp ORDER BY tax) FROM payments",
            "SELECT SUM(user_id) OVER (ORDER BY timestamp PARTITION BY tax) FROM payments",
            "SELECT CUMSUM(user_id) OVER (PARTITION BY timestamp ORDER BY tax) FROM payments"
        ],
        "correct": [
            1
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 2,
        "question": "Given the table `clicks`, which SQL query calculates the cumulative sum of `amount` partitioned by `revenue` ordered by `session_id`?",
        "options": [
            "SELECT SUM(amount) OVER (PARTITION BY revenue ORDER BY session_id) FROM clicks",
            "SELECT CUMSUM(amount) OVER (PARTITION BY revenue ORDER BY session_id) FROM clicks",
            "SELECT SUM(amount) PARTITION BY revenue ORDER BY session_id FROM clicks",
            "SELECT SUM(amount) OVER (ORDER BY revenue PARTITION BY session_id) FROM clicks"
        ],
        "correct": [
            0
        ],
    ...
    },
    {
        ...
        "type": "single",
        "difficulty": 3,
        "question": "Given the table `clicks`, which SQL query calculates the cumulative sum of `amount` partitioned by `discount` ordered by `timestamp`?",
        "options": [
            "SELECT SUM(amount) OVER (PARTITION BY discount ORDER BY timestamp) FROM clicks",
            "SELECT CUMSUM(amount) OVER (PARTITION BY discount ORDER BY timestamp) FROM clicks",
            "SELECT SUM(amount) PARTITION BY discount ORDER BY timestamp FROM clicks",
            "SELECT SUM(amount) OVER (ORDER BY discount PARTITION BY timestamp) FROM clicks"
        ],
        "correct": [
            0
        ],
